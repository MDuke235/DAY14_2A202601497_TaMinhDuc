...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        The retrieval metrics are intentionally excluded because they diagnose
        a different pipeline stage and do not change the original pass rule.
        """
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial evaluator. Score the answer for every "
            "rubric criterion from 0.0 to 1.0. Return only a JSON object "
            "mapping criterion names to numeric scores.\n\n"
            f"Question:\n{question}\n\n"
            f"Answer:\n{answer}\n\n"
            f"Rubric:\n{rubric_lines}"
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        positional_bias = (
            len(averages) > 1
            and all(averages[0] > score for score in averages[1:])
        )
        return {
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        Empty inputs have an average of 0.0, keeping the method deterministic.
        """
        def metric_average(results: list, metric: str) -> float:
            if not results:
                return 0.0
            return sum(float(getattr(item, metric)) for item in results) / len(results)
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        Suggestions are matched by row; if the list is shorter than failures,
        the row uses a general review action.
        """
        header = (
            "| Failure ID | Type | Root Cause | Suggested Fix | Status |\n"
            "|---|---|---|---|---|"
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            suggestion = (
                ...
                else "Review the full trace and add a targeted regression case"
            )
            rows.append(
                "| "
                + " | ".join(
    ...
    def generate_improvement_suggestions(
        ...
        category_actions = {
            "hallucination": "Implement a grounding check that rejects claims unsupported by retrieved policy text",
            "irrelevant": "Add intent-specific prompt examples and routing tests for commonly confused student-service requests",
            "incomplete": "Add required-field checklists for deadlines, conditions, exceptions, and responsible offices",
            "off_topic": "Improve intent detection and add an explicit out-of-scope response policy",
            "refusal": "Calibrate safety guardrails with valid in-scope examples to reduce unnecessary refusals",
            "unknown": "Inspect answer and retrieval traces, then add a targeted regression case",
        }
        suggestions = [
        ...
        general_actions = [
            "Tune chunking, query expansion, and reranking; verify gains with Context Recall and Context Precision",
            "Add the lowest-scoring traces to the golden dataset and run regression evaluation before deployment",
            "Calibrate automatic metric thresholds against human-reviewed Student Services answers",
        ]
